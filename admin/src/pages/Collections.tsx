/**
 * Collections management admin page.
 *
 * Table view with inline editing, product assignment,
 * manual product ordering, and CRUD operations for collections.
 *
...
export function Collections() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    queryFn: () => api.getCollections({ limit: 100, status: statusFilter || undefined }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: { name: string; description?: string; sort_order?: number }) => api.createCollection(data),
    onSuccess: () => {
  ...
  const addProductsMutation = useMutation({
    mutationFn: ({ id, product_ids }: { id: string; product_ids: string[] }) =>
      api.addCollectionProducts(id, product_ids),
    onSuccess: () => {
      refreshDetail();
  ...
  const removeProductMutation = useMutation({
    mutationFn: ({ id, productId }: { id: string; productId: string }) =>
      api.removeCollectionProduct(id, productId),
    onSuccess: () => {
      refreshDetail();
    },
  });
  ...
  const reorderMutation = useMutation({
    mutationFn: ({ id, items }: { id: string; items: Array<{ product_id: string; sort_order: number }> }) =>
      api.reorderCollectionProducts(id, items),
    onSuccess: () => {
      refreshDetail();
    },
  });
  ...
  const refreshDetail = () => {
    if (selectedCollection) {
      api.getCollection(selectedCollection.id).then(setSelectedDetail);
    }
  };

  const handleSelectCollection = async (col: Collection) => {
    setSelectedCollection(col);
    const detail = await api.getCollection(col.id);
  ...
  const handleAddProducts = () => {
    if (!selectedCollection || selectedProductIds.length === 0) return;
    addProductsMutation.mutate({ id: selectedCollection.id, product_ids: selectedProductIds });
  };
  ...
  const handleSortOrderChange = (productId: string, sort_order: number) => {
    if (!selectedDetail) return;
    const items = selectedDetail.products.map((p: any) => ({
      product_id: p.id,
      sort_order: p.id === productId ? sort_order : p.sort_order,
    }));
    reorderMutation.mutate({ id: selectedDetail.id, items });
  };

  const confirmDelete = (col: Collection) => {
    if (window.confirm(`Delete collection "${col.name}"? This will unlink all products.`)) {
  ...
  const productsInCollection = selectedDetail?.products || [];
  const allProductsList = allProducts.data?.items || [];
  const filteredProducts = allProductsList.filter(
    (p) =>
      !productsInCollection.some((pc: any) => pc.id === p.id) &&
  ...
  return (
    <div>
      ...
      {/* Create Modal */}
      <Modal open={createModal} onClose={() => setCreateModal(false)} title="New Collection" size="md">
      ...
      <Modal
        open={!!selectedDetail}
        onClose={() => { setSelectedCollection(null); setSelectedDetail(null); }}
        title={selectedDetail?.name || 'Collection'}
        size="lg"
      >
        {selectedDetail && (
            ...
            <div className="p-3 rounded-lg" style={{ border: '1px solid var(--border)' }}>
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>
                  Products ({productsInCollection.length})
                </h4>
              </div>
              ...
              {productsInCollection.length === 0 ? (
                ...
                <div className="space-y-2 mb-4">
                  {productsInCollection.map((p: any) => (
                    <div
                      key={p.id}
                      className="flex items-center gap-3 p-2 rounded-lg"
                      style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border-subtle)' }}
                    >
                      <GripVertical size={14} style={{ color: 'var(--text-muted)' }} className="flex-shrink-0" />
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <span className="font-mono text-sm truncate flex-1">{p.title}</span>
                        <input
                          type="number"
                          ...
                          onBlur={(e) => {
                            ...
                            if (!isNaN(val) && val !== p.sort_order) {
                              handleSortOrderChange(p.id, val);
                            }
                          }}
                          min="0"
                          className="w-16 px-2 py-1 font-mono text-xs rounded focus:outline-none focus:ring-2"
                          style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text)' }}
                          title="Sort order"
                        />
                      </div>
